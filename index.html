...
$routes       = array_filter( explode('/',$first_route[0]) );
  ...
  if( !isset($route[0]) && $_SESSION["userlogin"] == true ){
    $route[0] = "neworder";
    $routeType= 0;
  }elseif( !isset($route[0]) && $_SESSION["userlogin"] == false ){
    $route[0] = "auth";
    $routeType= 1;
  }elseif( $route[0] == "auth" && $_SESSION["userlogin"] == false ){
    $routeType= 1;
  }else{
    $routeType= 0;
  }
...
if( $settings["recaptcha"] == 1 ){
    	$captcha = false;
  //}elseif( ( $settings["recaptcha"] == 2 && ( route(1) == "register" || route(0) == "resetpassword" ) ) ){
	}else{
    	$captcha = true;
  	}
  ...
  if( route(0) != "admin" && route(0) != "ajax_data" ){
	...
	if( $_SESSION["userlogin"] != 1  || $user["client_type"] == 1  ){
      ...
      array(
            ...
            'ticketCount'=>new_ticket($user['client_id']),'paymentsList'=>$methodList,'bankPayment'=>$bankPayment["method_type"],'bankList'=>$bankList,
